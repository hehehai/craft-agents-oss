								...
								>
									...
									>
										...
										{connectionUnavailable ? (
											...
											connectionsByProvider.map(
												([providerName, connections], index) => (
																...
																<DropdownMenuSub key={conn.slug}>
																	...
																	{isAuthenticated && (
																		<StyledDropdownMenuSubContent className="min-w-[220px]">
																			...
																			{(conn.models || ANTHROPIC_MODELS).map(
																				(model) => {
																					const modelId =
																						typeof model === "string"
																							? model
...
function WorkingDirectoryBadge({
	...
	const refreshGitState = React.useCallback(() => {
		if (!window.electronAPI) return;
	...
	// Refresh git info when working directory changes
	React.useEffect(() => {
		refreshGitState();
	}, [refreshGitState]);

	// Refresh git diff stats when current session task finishes
	React.useEffect(() => {
		if (!sessionId) return;
		...
		showGitDiffStats && gitDiffStats ? (
			<span className="inline-flex items-center gap-1 min-w-0">
				...
				<span className="shrink-0 tabular-nums">
					(<span className="text-success">+{gitDiffStats.added}</span>,
					<span className="text-destructive">-{gitDiffStats.removed}</span>)
				</span>
			</span>
		) : (
			folderName
